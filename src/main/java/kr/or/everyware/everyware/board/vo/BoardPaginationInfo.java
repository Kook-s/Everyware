...
@Setter
@ToString
public class BoardPaginationInfo<T> {
...
public BoardPaginationInfo() {}
	
	
	//PaginationInfoVO 객체를 만들 때, 한 페이지당 게시글 수와 페이지 클록 수를
	// 원하는 값으로 초기화 할 수 있도록 Custom 생성자 생성
	public BoardPaginationInfo(int screenSize, int blockSize) {
		super();
	...
	public String getPagingHTML() {
		...
		if(startPage > 1) {
		html.append("<li calss='page-item'><a href='' class='page-link' data-page='"+(startPage - blockSize)+"'>Prev</a></li>'");
		...
		for(int i = startPage; i<=(endPage < totalPage ? endPage : totalPage); i++){
			if(i == currentPage) {
				...
				html.append("<li clas='page-item'><a href='' class='page-link' data-page ='"
							+ i +"'>"+ i + "</a></li>");
			}
